		...
		if (StringUtil.isNotBlank(path)) {
			...
			for (String p : paths) {
				if (StringUtil.isNotBlank(p)) {
					...
					String delpath = ctxPath + File.separator + p;
					File fileDelete = new File(delpath);
					if (fileDelete.exists() && fileDelete.isFile()) {
						fileDelete.delete();
						return true;
					} else {
						logger.info(delpath+" 不存在或不是文件");
					}
				}
			}
		}
...
//		log.setTSUser(ResourceUtil.getSessionUser());
	 ...
	 */
	@Override
	public void addAppLog(String logcontent, Short loglevel, Short operatetype, TSUser u) {
		HttpServletRequest request = ContextHolderUtils.getRequest();
		TSLog log = new TSLog();
		log.setLogcontent(logcontent);
		log.setLoglevel(loglevel);
	 ...
	 */
	@Override
	public String getTypeName(String typecode, String typegroupcode) {
		List<TSTypegroup> groups = this.findByProperty(TSTypegroup.class, "typegroupcode", typegroupcode);
		List<TSType> type = null;
		if (groups != null && groups.size() > 0) {
			type = commonDao.findHql("from TSType where typecode = ? and typegroupid = ?", typecode, groups.get(0).getId());
		}
		String typeName = typecode;
		if (type != null && type.size() > 0) {
			typeName = type.get(0).getTypename();
		}
		return typeName;
	}
	...
	public String getAuthFilterJS() {
		...
		if (ResourceUtil.getSessionUser().getUserName().equals("admin") || ResourceUtil.getSessionUser().getUserName().equals("zhouwr") || !Globals.BUTTON_AUTHORITY_CHECK) {
			return "";
		}else{
			HttpServletRequest request = ContextHolderUtils.getRequest();
