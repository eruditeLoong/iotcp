...
public class UserServiceImpl extends CommonServiceImpl implements UserService {
	 ...
	 */
	@Override
	public List<TSUser> getUsersByRolecode(String rolecode) {
		List<TSUser> users = new ArrayList<TSUser>();
		/**
		 * select * from t_s_user u, t_s_role r, t_s_role_user ru where u.id=ru.userid and ru.roleid=r.id and r.rolecode='deptLeader';
		 * select u.* from t_s_role r left join t_s_role_user ru on r.id=ru.roleid left join t_s_user u on ru.userid=u.id where r.rolecode='deptLeader';
		 */
		String hql = "select new org.jeecgframework.web.system.pojo.base.TSUser(u.id,u.realName,u.userName) from TSUser u, TSRole r, TSRoleUser ru where u.id=ru.TSUser.id and ru.TSRole.id=r.id and r.roleCode=?";
		users = this.commonDao.findHql(hql, rolecode);
		return users;
	}
