...
<body>
	<div id="deviceTitle" class="easyui-panel" title="场景列表" style="width: auto;">
		<a id="resetBtn" onclick="setDefaultScene();" class="ace_button" style="background-color: #AD3E74"> 设为默认展示 </a>
	...
	<script type="text/javascript">
		...
		function setDefaultScene() {
			var sceneId = zTree.getCheckedNodes(true)[0].id;
			if (sceneId == undefined) {
				$.dialog.tips("请选择场景！", 3, 'tips.gif', function() {
				});
			} else {
				$.getJSON("sceneController.do?doSetDefectScene&id=" + sceneId, function(result) {
					if (result.success) {
			...
			case 'scene':
				...
				$.getJSON("sceneController.do?getDeployFileUrl&sceneId=" + treeNode.id, function(result) {
					if (result.success) {
						...
						var url = result.obj;
						sceneDeployFrame.initInstanceScene(treeNode.id, treeNode.name, treeNode.deployInfo, url);
					}
				});
				break;
			case 'deviceConf':
		...
		function searchDevice(deviceBy) {
			...
			sceneDeployFrame.searchDeviceById(deviceBy);
		}
